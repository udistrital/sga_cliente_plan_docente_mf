...
export class AsignarPtdComponent implements OnInit, AfterViewInit {
  ...
  displayedColumns: string[] = ["docente", "identificacion", "tipo_vinculacion", "periodo_academico", "soporte_documental", "gestion", "estado", "enviar"];
  ...
  accionGestion(event: any) {
    ...
    this.sgaPlanTrabajoDocenteMidService.get(`plan/?docente=${event.rowData.docente_id}&vigencia=${event.rowData.periodo_id}&vinculacion=${event.rowData.tipo_vinculacion_id}`).subscribe((res: any) => {
      ...
      if (this.coordinador && (event.rowData.estado === "Enviado a coordinación")) {
  ...
  checknloadRelatedPTD(otherPTD: string[]) {
    ...
    otherPTD.forEach(doc_per_vinc => {
      this.sgaPlanTrabajoDocenteMidService.get('plan/'+doc_per_vinc).subscribe(res => {
  ...
  loadAsignaciones() {
    if (this.coordinador) {
      this.sgaPlanTrabajoDocenteMidService.get('asignacion/?vigencia=' + this.periodo.Id).subscribe({
      ...
      this.userService.getPersonaId().then(id_tercero => {
        this.sgaPlanTrabajoDocenteMidService.get('asignacion/docente?docente=' + id_tercero + '&vigencia=' + this.periodo.Id).subscribe({
          next: (resp: RespFormat) => {
            if (checkResponse(resp) && checkContent(resp)) {
              this.dataSource = new MatTableDataSource(resp.Data);
            } else {
    ...
    if (estado) {
      this.planTrabajoDocenteService.get('plan_docente/' + id_plan).subscribe({
        next: (res_g) => {
          ...
          this.planTrabajoDocenteService.put('plan_docente/' + id_plan, res_g.Data).subscribe({
            next: (res_p) => {
